...
-- Konsolidiert: Basis-Schema + alle Migrationen
...
-- contact_role_id ist nullable, da Verträge auch ohne Kontaktrolle
-- angelegt werden können (z.B. beim Erstellen via Vertragsvorlage).
-- ============================================================

create table contracts (
  id                           uuid primary key default gen_random_uuid(),
  tenant_id                    uuid not null references tenants(id),
  contact_role_id              uuid references contact_roles(id),   -- nullable
  created_by                   uuid references auth.users(id),
  ...
  deleted_at                   timestamptz,
  owner_id                     uuid references auth.users(id),
  deal_value_eur               numeric(10,2),
  mrr_value                    numeric(10,2),
  qualified_at                 timestamptz,
  demo_booked_at               timestamptz,
  demo_happened_at             timestamptz,
  won_at                       timestamptz,
  lost_at                      timestamptz,
...
create index idx_contracts_contact_role on contracts(contact_role_id) where contact_role_id is not null;
...
-- und erweiterten channel-Enum (migration-communications)
-- ============================================================
...
-- ============================================================
-- 13. APP – Emails
-- Enthält neue Kategorien (migration-email-categories) +
-- linked_*-Spalten (migration-email-links)
-- ============================================================
...
-- ============================================================
-- ACTIVITIES – Logging all touchpoints on contacts and deals
-- ============================================================
...
-- ============================================================
-- SALES_TARGETS – Per-user, per-period quota settings
-- ============================================================
...
-- ============================================================
-- 28. STORAGE – documents Bucket + RLS
-- ============================================================
...
-- ============================================================
-- 30. EMAIL CLASSIFICATION – updated_at trigger + training view
-- ============================================================

create trigger trg_email_classification_updated_at
  before update on email_classification
  for each row execute function set_updated_at();

-- Training-data view: emails with outcome + human label
